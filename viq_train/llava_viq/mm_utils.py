...
from llava_viq.constants import IMAGE_TOKEN_INDEX
...
def get_anyres_image_grid_shape_ours(image_size, grid_pinpoints, patch_size):
    """
    Calculate the shape of the image patch grid after the preprocessing for images of any resolution.
...
def process_anyres_highres_image(image, processor, grid_pinpoints):
    processor2 = None
    ...
    return torch.stack(image_patches, dim=0), image_padded
...
def process_anyres_image(image, processor, grid_pinpoints):
    ...
    image_patches = [processor.preprocess(image_patch, return_tensors='pt')['pixel_values'][0]
                     for image_patch in image_patches]
    image_padded = processor.preprocess(image_padded, return_tensors='pt')['pixel_values'][0]
    image_patches = torch.stack(image_patches, dim=0)
    return image_patches, image_padded
...
def process_images(images, image_processor, model_cfg):
    ...
    if image_aspect_ratio == "highres":
        ...
        for image in images:
            image_lowres, image_highres = process_anyres_image(image, image_processor, model_cfg.image_grid_pinpoints)
            new_images.append(image_lowres)
            new_images.append(image_highres)
    elif image_aspect_ratio == "anyres_highres":
        image_processor.do_resize = False
        image_processor.do_center_crop = False
        for image in images:
            image = resize_images(image)
            image_lowres, image_highres = process_anyres_highres_image(image, image_processor, model_cfg.image_grid_pinpoints)
            new_images.append(image_lowres)
            new_images.append(image_highres)
