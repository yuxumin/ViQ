...
class BinarySphericalQuantizer(nn.Module):
    def __init__(self, in_dim, embed_dim, beta, gamma0, gamma, zeta,
                 input_format='bchw', 
        ...
        has_projections = (in_dim != embed_dim)
        self.project_in = nn.Linear(in_dim, embed_dim) if has_projections else nn.Identity()
        output_feat_dim = default(output_feat_dim, in_dim)
        has_projections = (embed_dim != output_feat_dim)
        self.project_out = nn.Linear(embed_dim, output_feat_dim, bias = projection_has_bias) if has_projections else nn.Identity()
    ...
    def forward(self, z):
        ...
        zq = zq * q_scale # equal to l2 norm

        # commit loss
        commit_loss = self.beta * torch.mean(((zq.detach() - z) ** 2).sum(dim=-1))

        zq = self.project_out(zq)
