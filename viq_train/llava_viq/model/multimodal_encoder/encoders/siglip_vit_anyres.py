...
if 'SIGLIP2' in os.environ:
    print("SIGLIP2 is set")
    SIGLIP2 = True
else:
    SIGLIP2 = False
...
class VisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list):
        ...
        x_return = x.split(slen, dim=1) #[(1, slen, c)]
    ...
    def forward(self, x, cal_attn_pool=False):
        if type(x) is list:
            ...
            if not cal_attn_pool:
                ...
                return x, image_sizes, cls_token
    ...
    if ckpt_path:
        ...
        if not teacher:
            ...
            if SIGLIP2:
                ...
                if pos_embed.shape[1] != model.pos_embed.shape[1]:
                    # siglip2 so400m
                    pos_embed = pos_embed.reshape(1, 24, 24, 1152).permute(0, 3, 1, 2)
                    pos_embed = torch.nn.functional.interpolate(pos_embed, size=(128, 128), mode='bicubic', align_corners=False)
                    pos_embed = pos_embed.permute(0, 2, 3, 1).reshape(1, -1, 1152)
                    print(f'interpolate model pos embed size to 128...')
                    new_state_dict['pos_embed'] = pos_embed
...
class SigLIPViTAnysizeWrapper(nn.Module):
    ...
    def load_model(self, device_map=None):
        ...
        if os.path.exists('/home/models/siglip_anyres_distilled.pth'):
            model_path = '/home/models/siglip_anyres_distilled.pth'
            print("Loading vision model from /home/models/siglip_anyres_distilled.pth")
        else:
            model_path = '/home/models/open_clip_pytorch_model.bin'
            print("Loading vision model from /home/models/open_clip_pytorch_model.bin")

        if VIT_WITH_GRAD:
            if SIGLIP2:
                self.vision_tower = create_siglip_vit(ckpt_path=model_path, model_name='siglip2_giant_patch16_384',
                                                  gradient_checkpointing=True)
            else:
                self.vision_tower = create_siglip_vit(ckpt_path=model_path, model_name='siglip2_so400m_patch16_384',
                                                  gradient_checkpointing=True)
        ...
        if SIGLIP2:
            self.vision_tower_teacher = create_siglip_vit(ckpt_path='/home/models/open_clip_pytorch_model.bin', model_name='siglip2_giant_patch16_384',
                                                      gradient_checkpointing=False, teacher=True)
        else:
            self.vision_tower_teacher = create_siglip_vit(ckpt_path='/home/models/open_clip_pytorch_model.bin', model_name='siglip2_so400m_patch16_384',
                                                      gradient_checkpointing=False, teacher=True)
    ...
    def forward_func(self, images, force_fix_size=False, cal_attn_pool=False):
        if type(images) is list:
            ...
            if h*w > 1024*1024:
                print('## very large image', images.shape)
    ...
    def forward_teacher_local(self, images, cal_attn_pool=False):
        with torch.no_grad():
            image_features, img_size, cls_token = self.vision_tower_teacher(images, cal_attn_pool=cal_attn_pool)
            if cls_token is not None:
                return image_features, img_size, cls_token
            else:
                return image_features, img_size
    ...
    def dummy_feature(self):
        if SIGLIP2:
            return torch.zeros(1, 1536, device=self.device, dtype=self.dtype)
        else:
